...
abstract class BaseActivity<VB : ViewBinding> : BaseFrameActivity<VB>(),
    ...
    override fun onDestroy() {
     ...
     fun initBannerAds(frameLayout: FrameLayout) {
        val singleAdaptiveBannerManager =
            AdaptiveBannerManager(
                this,
                getString(R.string.banner_01),
                getString(R.string.banner_02)
            )
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_PERM_DELETE && resultCode == RESULT_OK) {
            ...
            Log.e("REQUEST_PERM_DELETE", removeItemSong.toString())
            showMessage(getString(R.string.delete_success))
            EventBus.getDefault().postSticky(removeItemSong?.let { EventDeleteSong(it) })
            removeItemSong = null
        }
        if (requestCode == REQUEST_PERM_DELETE_MULTIPLE_FILE && resultCode == RESULT_OK) {
            ...
            Log.e("REQUEST_PERM_DELETE", removeItemSong.toString())
            EventBus.getDefault().postSticky(EventRefreshDataWhenDelete(true))
        }
    }
    ...
    protected open fun sendEmail(content: String) {
        val to = arrayOf("3jhung134267@gmail.com")
        val emailIntent = Intent(Intent.ACTION_SEND)
        emailIntent.data = Uri.parse("mailto:")
        emailIntent.type = "message/rfc822"
        emailIntent.putExtra(Intent.EXTRA_EMAIL, to)
        emailIntent.putExtra(Intent.EXTRA_SUBJECT, "Feedback Download Mp3 Player")
        emailIntent.putExtra(Intent.EXTRA_TEXT, content)
        try {
            startActivity(Intent.createChooser(emailIntent, "Feedback Download Mp3 Player"))
    ...
    fun connectionChange(event: EventChangeNetwork) {
        if (event.isChange) {
    ...
    fun message(bus: EventDeleteSong) {
