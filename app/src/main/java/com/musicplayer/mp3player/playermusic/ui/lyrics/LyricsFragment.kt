...
import com.musicplayer.mp3player.playermusic.eventbus.BusChangePager
...
import com.musicplayer.mp3player.playermusic.eventbus.BusShowLyric
...
import com.musicplayer.mp3player.playermusic.equalizer.databinding.FragmentLyricsBinding
...
//        loadBanner(frameBannerAds)
        ...
        binding.btnCancelFindLyrics.setOnClickListener {
            EventBus.getDefault().postSticky(BusChangePager(1))
        ...
        binding.tvLyrics.setOnClickListener {
            EventBus.getDefault().postSticky(BusShowLyric(true))
        }
    ...
    fun onMessageEvent(event: BusUpdateLyric?) {
        if (event != null) if (event.isSaveLyric) {
            updateLyrics()
            EventBus.getDefault().postSticky(BusUpdateLyric(false))
        }
    }
    ...
    @Subscribe(threadMode = ThreadMode.MAIN, sticky = true)
    fun refreshThumb(event: BusRefreshData) {
        setAvtThumbnail()
    }
